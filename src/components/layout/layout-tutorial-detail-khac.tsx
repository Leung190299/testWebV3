...
const LayoutTutorialKhac: FC<Props> = ({ tutorial, className, ads, goBack }) => {
        ...
        <div className="container md:pb-12 md:pt-10 xl:pb-14">
          <div className="content-detail">
            ...
            <div className="txt-detail">
              <div className="flex flex-wrap">
                <div className="w-2/3 pr-5">
                  <div className="detail">
                    {data.description !== undefined && <div dangerouslySetInnerHTML={{ __html: data.description }}></div>}
                  </div>

                  {data.content_step_mutili &&
                    data.content_step_mutili.length <= 2 &&
                    data.content_step_mutili.map((content, index) => (
                      <div key={index} className="mt-12 step-detail">
                        <h2 className="text-2xl font-bold">{content.title}</h2>
                        <div className="flex overflow-auto scrollbar-hide md:py-9 xl:py-8 snap-mandatory snap-x touch-pan-x">
                          {content.steps.map((slide, index) => (
                            <SectionSliderStep
                              key={index}
                              src={slide.image}
                              name={slide.title}
                  ...
                  {data.textEnd && data.content_step_mutili && data.content_step_mutili.length <= 2 && (
                    <>
                      <div className="text-xl font-bold  leading-7">Lời kết</div>
                      <div className=" text-base font-normal  leading-normal">{data.textEnd}</div>
                    </>
                  )}
                  {data.file && data.content_step_mutili && data.content_step_mutili.length <= 2 && (
                  ...
                  {data.content_step_mutili && data.content_step_mutili.length <= 2 && (
                    <div className="huuich text-center p-4 mt-6 pt-10">
                      ...
                      <ul className="items-center justify-center">
                        <li>
                          <button
                            className={isActive ? 'bg-salmon clc-huuich flex justify-center' : 'clc-huuich flex justify-center'}
                            onClick={handleClick}
                          >
                            <img src="/tutorial/emoj-1.png" alt="" className="mr-4" />
                            Không hữu ích
                          </button>
                        </li>
                        <li>
                      ...
                      <div className="content-sidebar relative">
                        ...
                        <div className="txt-sidebar absolute w-full h-full top-0 left-0 p-12 px-8">
                          ...
                          <div className="btn-bar">
                            <a
                              href="https://itel-web.vercel.app/sim"
                              className="btn-primary btn rounded-full p-4 px-15 md:w-[174px] h-full"
                            >
                              Gét gô!
                            </a>
                          </div>
              ...
              {data.content_step_mutili && data.content_step_mutili.length > 2 && (
                <div className="w-full mt-12 step-detail">
                  <h2 className="text-center text-neutral-900 text-[32px] font-bold">
                    Chi tiết cách cài đặt APN với từng dòng máy điện thoại
                  </h2>
                  <div className="py-4 pl-4 md:pl-0 border-b border-b-neutral-200 conten-list-slide">
                    <ul className="flex  justify-start md:justify-center gap-4 overflow-auto scrollbar-hide">
                      {data.content_step_mutili.map((content, index) => (
                        <li
                          key={index}
                          onClick={() => setTabIndex(index)}
                          className={clsx(index != tabIndex ? 'btn-secondary' : 'btn-primary', 'btn ')}
                        >
                          {content.title}
                        </li>
                      ))}
                  ...
                  <p className="text-center text-stone-900 mt-4 text-base font-normal leading-normal">
                    Lựa chọn dòng máy điện thoại của Bạn để xem chi tiết các bước cài đặt tương ứng nhé!
                  </p>
                  <div className="relative">
                    {data.content_step_mutili.map((content, index) => (
                      <div key={index} className={clsx('conten-list-slide', index == tabIndex ? 'flex' : 'hidden')}>
                        <div className={clsx('flex overflow-auto scrollbar-hide md:py-9 xl:py-8 snap-mandatory snap-x touch-pan-x')}>
                          {content.steps.map((slide, index) => (
                            <SectionSliderStep
                              key={index}
                              src={slide.image}
...
export default LayoutTutorialKhac;
