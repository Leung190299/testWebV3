...
const FilmFeaturePage: NextPage<PageProps> = ({ data }) => {
  return (
    <div>
      <Head>
        <title>iFilm</title>
      ...
      <section className="bg-neutral-900 xl:py-20 md:py-10 py-8">
        <div className="container">
          <div className="xl:block hidden" id="film-list">
            <SectionFilmList label="Phim LẺ HẤP DẪN" filmData={data.slice(0, 4)} isDisplayButtonFilter={false} />
          </div>
          <div className="xl:hidden block">
            <SectionFilmFeature label="Phim LẺ HẤP DẪN" filmData={data.slice(0, 4)} />
          </div>
          <div className="mt-10 md:mt-20">
            <SectionFilmList label="DANH SÁCH PHIM LẺ" filmData={data} className="gap-[1.125rem]" isHorizontal />
...
const getStaticProps: GetStaticProps<PageProps> = async () => {
  ...
  return { props: { data: getMultipleRandom(data, 15) } };
};
FilmFeaturePage.getLayout = function (page) {
  return (
    <>
      <LayoutIFilm pageMobileTitle="Phim lẻ hấp dẫn" footerClassName="bg-neutral-700">
        {page}
      </LayoutIFilm>
      <ChatBoxLazy />
    </>
