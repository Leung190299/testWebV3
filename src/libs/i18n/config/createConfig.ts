...
export const createConfig = (userConfig: UserConfig): InternalConfig => {
  ...
  if (nonExplicitSupportedLngs) {
    ...
    if (typeof fallbackLng === 'string') {
      ...
      combinedConfig.fallbackLng = Object.entries(combinedConfig.fallbackLng).reduce<FallbackLngObjList>((acc, [l, f]) => {
  ...
  if (!process.browser && typeof window === 'undefined') {
    ...
    if (!hasCustomBackend) {
      ...
      if (typeof localePath === 'string') {
        ...
        combinedConfig.backend = {
          addPath: (locale: string, namespace: string) => localePath(locale, namespace, true),
          loadPath: (locale: string, namespace: string) => localePath(locale, namespace, false)
        };
      } else {
